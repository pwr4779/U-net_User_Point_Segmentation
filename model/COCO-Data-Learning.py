...
if __name__ == "__main__":
    ...
    backfiles = glob.glob('/tmp/pycharm_project_368/Back_Point2/*')
    ...
    for j in range(m):
        ...
        for i in range(int(len(pointfiles)*1/10)):
            ...
            backpath = pointfiles[i].replace('/Person_Point2', '/Back_Point2')
    ...
    train_x = train_x.reshape(len(train_x), 288, 480, 5).astype('float32')/255.0
    valid_x = valid_x.reshape(len(valid_x), 288, 480, 5).astype('float32')/255.0
    ...
    results = model.fit(train_x, train_mask_y, validation_data=(valid_x, valid_mask_y), epochs=50, batch_size=16, verbose=1, callbacks=[early_stopping])
    model.save("COCO_2point_21kernel_v2.h5")
    print('finish')