...
if __name__ == "__main__":
    ...
    dirNames = glob.glob('/tmp/pycharm_project_368/DAVIS/Annotations/480p/*')
    #print(dirNames)
    #for i in range():
    ...
    for i in range(len(dirNames)):
        fileNames = glob.glob(dirNames[i]+"/*")
        for file in fileNames:
            # print(file)
            image = np.array(pilimg.open(file))
            #print(image)
            TPL = targetPixelList(image)
            patch = createPointLabel(TPL, image)
            dirpath = dirNames[i].replace('/480p','/Point')
            if not (os.path.isdir(dirpath)):
                os.makedirs(os.path.join(dirpath))
            path =file.replace('/480p','/Point')
